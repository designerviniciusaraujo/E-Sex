...
var sF$ = (function () {
  ...
  function fnLoginUserName(NameUser, PicUser) {
    ...
    if (NameUser == "") {
      ...
      jQuery('.loginInfo').html("<span class='col-small-12 HeaderSocialLoginUserName'>Olá " + NameUser + ",</span> <span class='col-small-12 HeaderSocialLoginUserName'><a href='#Logout' onclick=FCLib$.fnClientLogout('',sF$.fnCliLogout)><span class='HeaderSocialLoginLogout'>&nbsp;(sair)</span></a></span>");
  ...
  function fnMostraDescontoHome(PrecoProd) {
    ...
    if(PrecoProd>=200.00){
  document.write("<span style=''>"+FormatPrecoReais(PrecoProd*0.85)+"</font></span>");
  }
  ...
  function fnMostraDescontoProdDet(PrecoProd) {
     if(PrecoProd>=200.00){
    document.write("<span>"+FormatPrecoReais(PrecoProd*0.85)+"</span>");
    }
    else if(PrecoProd>=0.00){
    document.write("<span>"+FormatPrecoReais(PrecoProd*0.90)+"</span>");
    }
  }
  ...
  // Produto Promotion Tag
  function fnShowEconomyFlag(ProdPriceNum, ProdPriceOri, idHTML) {
    var el = document.getElementById(idHTML);
    if (ProdPriceNum != ProdPriceOri && el) {
      var iPercentual = fnFormatNumber(((ProdPriceOri - ProdPriceNum) / ProdPriceOri) * 100);
      ...
      if (iPercentual >= 50) {
        ...
        sColorRect = "#4BDD14"; sColorPolygon = "#e20b7c";
      }
  ...
  function fnBadgeProdZF(iPrecoOri, iPrecoNum, sLacamento, sDestaque) {
      //if(iPrecoOri > iPrecoNum){ return document.write("<div class='zFProdSeloProd'><img src='"+ FC$.PathImg +"icones/faixas_destaques.svg' /></div>")}
      if (iPrecoOri > iPrecoNum) { return document.write("<div class='zFProdSeloProd'><span class='Prom'>PROMOÇÃO</span></div>") }
      else if (sLacamento !== "") { return document.write("<div class='zFProdSeloProd'><span class='LancFlag'>LAN&#xC7;AMENTO</span></div>") }
      else if (sDestaque !== "") { return document.write("<div class='zFProdSeloProd'><span class='DestFlag'>DESTAQUE</span></div>") }
    //console.log(iPrecoOri, iPrecoNum, sLacamento, sDestaque);
  }

  function MostraParcelaPagSeguroHome(PrecoProd, MaxParcelas, IDProduto) {
    var parcelas = document.getElementById("ParcelasPagSeg" + IDProduto);
    var ComSem;
    ...
    parcelas.innerHTML = "<span style='color: #5c5c5c;'>ou&nbsp;" + MaxParcelas + "x" + ComSem + " de&nbsp;<b>" + FormatPrecoReais(PrecoProd * FatorPagSeguro[MaxParcelas - 1]) + "</b></span><br>";
  }
...
function fnFooter() {
  ...
  if (getBodyClass === "FCProduct ProductList") {
    var domColumn = document.getElementById('idFCContent');
    var domRow = document.getElementById('idFCLeftContentRight');

    if (domRow) domRow.setAttribute('class', 'row');
    if (domColumn) domColumn.setAttribute('class', 'c-s-8 c-m-9 c-l-9');
  }
  ...
  if (FC$.Page == "Products" && ListVerify) {
    document.querySelector('#idFCContent').setAttribute('class', 'c-xs-12 c-s-12 c-m-9 c-l-10 DispParent');
  };
...
function fnShowGlobalSignin() {
  ...
  if (oImgGlobalSign) {
    ...
    if (typeof FC$.FacebookSigninID != "undefined") {
      sImgs += "|  &nbsp;Acesse via &nbsp;<img src='" + FC$.PathImg + "login_facebook.svg' class='FacebookSigninClass' data-loginsuccess='fnLoginShowUserName'> ";
      bFacebookLogin = true;
    }
    if (typeof FC$.GoogleSigninID != "undefined") {
      sImgs += "<img src='" + FC$.PathImg + "login_google.svg' class='GoogleSigninClass' data-loginsuccess='fnLoginShowUserName'>";
      bGoogleLogin = true;
    }
