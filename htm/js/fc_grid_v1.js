//FastCommerce Grid - zFast [04.09.2015] v1.02
function fnErrorImagemProd(img){ img.onerror=null; img.style.display = 'none';}
var FCGrid$ = function () {
  "use strict";
  ...
  var fn = {
      ...
      for (var i=0;i<=FC_MaxImages;i++)
      {
        if(i===0)
        {
          ...
          sHtmlZoom+="<a href="+imgAmpMini+" title=\""+ fn.getNameProduct() +"\" class=MagicZoomPlus id=zoom2 rel=\"selectors-class:active; zoom-width:350px; zoom-height:350px; selectors-change:mouseover;\"><img src="+ imgDetMini +"></a><br>"
                    /*+"<div class=\"FCGridBtnZoomContent\"><p>Toque na imagem para dar zoom</p></div>"*/
                    +"<div class=\"FCGridBtnZoomContent\"><p>Passe o mouse para ampliar a imagem</p> <a class=\"FCGridBtnZoom\" onclick=\"MagicZoomPlus.expand(zoom2); return false;\" href=\"#\"> Ampliar imagem</a>"+ oHTMLImgAltDefinicao +"</div>"
                    +"<a href=\""+imgAmpMini+"\" rel=\"zoom-id:zoom2;\" rev=\""+ imgDetMini +"\"><img src=\""+ imgDetMini +"\" width=65 height=65 class=ZoomIMG2 onerror=fnErrorImagemProd(this)></a>";
        }
        else{
          imgDetMini=FC$.PathPrd+novoArray[i];
          imgAmpMini=FC$.PathPrd+novoArrayAmp[i];
          sHtmlZoom+="<a href="+imgAmpMini+" rel='zoom-id:zoom2;' rev="+ imgDetMini +"><img src="+ imgDetMini +" width=65 height=65 class=ZoomIMG2 onerror=fnErrorImagemProd(this) onload=\"this.style.display='block'\" style='display:none'></a>";
        }
      }
      document.getElementById(id).innerHTML= "<div class=\"FCGridZoomMain\">"+ sHtmlZoom +"</div>";
    ...
    */    
    ...
    availableBuyProduct: function(product, sParamsGrid){
      fn.floatBuyButton(product, sParamsGrid);
      ...
      if(options.shippingUpdate){
        ...
        if(oZipField && oShippingValues){
          if(isView){
            ...
            oButtonShipping.onclick = function() {
               alert("Selecione primeiro as opções do produto.");
            };
    ...
    /* new */
    floatBuyButton: function(product, sParamsGrid){
      var oButtonFloat = document.getElementById("idProdDetLeftBuyZF");
      if(!oButtonFloat)return false;
      if(product){
        if(parseInt(product.estoque)<=0){
          oButtonFloat.setAttribute("class", "");
          oButtonFloat.innerHTML="<img src=\""+ FC$.PathImg + options.btnSoldOut +"\" />";
          oButtonFloat.onclick=null;
          oButtonFloat.onclick=function(){
            fn.popupSoldOutProduct(sParamsGrid);
          };
        }
        else if(parseInt(product.estoque)>0 && parseFloat(product.priceNum) === 0){
          oButtonFloat.setAttribute("class", "FCBtnGrid FCBtnConsultenos");
          oButtonFloat.innerHTML="<img src=\""+ FC$.PathImg + options.btnContactUSImg +"\">";
          oButtonFloat.onclick=null;
          oButtonFloat.onclick=function(){
            fn.consultUsAboutProduct(sParamsGrid);
          };
    ...
    for(var i=iNextNivel; i< aDestinosDescritores.length; i++){
      ...
      if(aDestinosDescritores.length > 0){
        ...
        if(!oElementExists){
          ...
          oNewElement.innerHTML = "<div class=\"FCStepGrid\">"
                                +   "<span class=\"FCStepGridTitle\">"+ sTitDescr + "</span>"
                                + "</div><span class=\"FCStepGridNumber\">"+ parseInt(i+1) +"</span>"+ sHtmlUL ;
  ...
  function fnInitProductList(aProductList){
    ...
    if(!settings.descriptorsActive || settings.descriptorsActive.length == 0)return false; //se exite subprodutos com erro no cadastro (ausencia de descritores)
    ...
    var fn_verify_stock_product = function (){
      ...
      for(var i=0; i < aProductListGrid.length; i++)
      {
        ...
        if(parseInt(_product.estoque) > 0 && parseInt(_product.priceNum) > 0){
          ...
          _notProductList.push(aProductListGrid[i]);
        }
      }
